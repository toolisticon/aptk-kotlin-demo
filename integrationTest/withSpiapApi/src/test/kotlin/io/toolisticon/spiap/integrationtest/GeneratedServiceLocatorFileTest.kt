...
class GeneratedServiceLocatorFileTest {
    ...
    @Test
    fun getServiceKeys_forDecimalCalculationOperation() {

        val operationServiceKeys : List<DecimalCalculationOperationServiceLocator.ServiceImplementation> = DecimalCalculationOperationServiceLocator.getServiceImplementations()
    ...
    fun locateAll_locateAllServices() {
        ...
        for ( service : DecimalCalculationOperation in services) {
            serviceIds.add(service::class.qualifiedName.toString())
        }

        // check if id referenced and fqn referenced service implementations have been found
        MatcherAssert.assertThat(serviceIds, Matchers.containsInAnyOrder(SquareDecimalOperationServiceImpl::class.qualifiedName.toString(), MultiplicationDecimalOperationImpl::class.qualifiedName.toString(), AdditionDecimalOperationImpl::class.qualifiedName.toString(), SubtractionDecimalOperationImpl::class.qualifiedName.toString(), DivisionDecimalOperationImpl::class.qualifiedName.toString()))
